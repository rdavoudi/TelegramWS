...
namespace TLSharp.Core
{
    public class TelegramClient : IDisposable
    {
        ...
        public TelegramClient(int apiId, string apiHash,
            ISessionStore store = null, string sessionUserId = "session_2", TcpClientConnectionHandler handler = null)
        {
        ...
        public async Task<int?> ImportContact(string phone, string first_name, string last_name)
        {
            ...
            var request = new TLRequestImportContacts() { Contacts = new TLVector<TLInputPhoneContact>(){ lists= lst } };
            await _sender.Send(request);
            await _sender.Receive(request);
